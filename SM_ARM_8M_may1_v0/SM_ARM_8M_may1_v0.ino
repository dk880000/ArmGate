...
RESET:
  ...
  { // If Up condition we write Down condition in EEPROM
    ...
    {
      MSpeed = i;
      for (int pwm = 0; pwm < 100; pwm++){       for (int c = 0; c < dc; c++) { if (c <= MSpeed)  PORTD = 0b01011000; else PORTD = 0b01010000;}    }
    }
            
    //Main moving 10 sec
    for (int i = 0; i < 215; i++) // timeout 9 sec
    {
      if (MSpeed == 0) break;
      else {}
      MSpeed = 25;//ceil(25 - i / 7);
      PORTD = 0b01011000;     data_rcvd = PORTD;   Serial.write(data_rcvd); //setd to serial PORTD state
  ...
  else
  {
    ...
    {
    MSpeed = i;
    for (int pwm = 0; pwm < 100; pwm++){       for (int c = 0; c < dc; c++) { if (c <= MSpeed)  PORTD = 0b01011100; else PORTD = 0b01010100;}    }
